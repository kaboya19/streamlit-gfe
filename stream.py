...
# Apply the function to each row to calculate the "Değişim" column
    ...
    kasım=np.round((((selected_group_data.iloc[:,0].loc["2024-11-01":"2024-11-24"].mean()/selected_group_data.iloc[:,0].loc["2024-10-01":"2024-10-24"].mean()))-1)*100,2)
    ...
    "Mayıs 24", "Haziran 24", "Temmuz 24","Ağustos 24","Eylül 24","Ekim 24","Kasım 24","Aralık 24"
        ...
        for col in weighted_indices.columns:
            for i in range(len(weighted_indices_aylık.index[2:])):

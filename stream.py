...
tabs=["Gıda Fiyat Endeksi","Özel Kapsamlı Endeksler","Mevsimsel Düzeltilmiş Göstergeler","Harcama Grupları","Metodoloji Notu","Bültenler","Bülten Aboneliği"]
tabs = option_menu(
    menu_title=None,
    options=["Gıda Fiyat Endeksi","Özel Kapsamlı Endeksler","Mevsimsel Düzeltilmiş Göstergeler", "Harcama Grupları", "Metodoloji Notu", "Bültenler", "Bülten Aboneliği"],
    menu_icon="cast",
    default_index=0,
...
# Hareketli aylık ortalama hesaplama
    ...
    tarihim=datetime.now(istanbul_tz).day
    if tarihim>24:
        tarihim=24
    if tarihim<10:
    ...
    from datetime import datetime,timedelta
    tarih=datetime.now(istanbul_tz).strftime("%Y-%m")
    onceki=(datetime.now(istanbul_tz)-timedelta(days=30)).strftime("%Y-%m")
    if selected_group=="WEB-GFE":
    ...
    if selected_group!="WEB-GFE":
        ...
        gfe=gfe.fillna(100)
        degisim24=np.round(((((hareketli_aylik_ortalama(gfe["GFE"])["Aylık Ortalama"].iloc[-1])/hareketli_aylik_ortalama(gfe["GFE"])["Aylık Ortalama"].loc[f"{oncekiyear}-{onceki}-24"]))-1)*100,2)
    ...
    go.Scatter(
        ...
        st.markdown(f"""
            <h3 style='text-align:left; color:black;'>
                {first_date} - {last_date} Değişimi: <span style='color:red;'>%{change_percent}</span><br>
                {ilk} - {son} Değişimi: <span style='color:red;'>%{np.round(aybasısonu,2)}</span><br>
                {month} Değişimi: <span style='color:red;'>%{ degisimgfe.round(2)} (Mevsimsel Düzeltilmiş:%{np.round(mason,2)})</span><br>
    ...
    aylıkenf.iloc[-1]=np.round(((((hareketlima["Aylık Ortalama"].iloc[-1])/hareketlima["Aylık Ortalama"].loc[f"{oncekiyear}-{onceki}-24"]))-1)*100,2)
    ...
    "Mayıs 24", "Haziran 24", "Temmuz 24","Ağustos 24","Eylül 24","Ekim 24","Kasım 24","Aralık 24"
        ...
        aylık_endeksler["Tarih"]=["2024-10","2024-11","2024-12","2025-01","2025-02","2025-03"]
        ...
        for col in endeksler1.columns:
            ...
            aylıık.loc["2024-11-30"]=((endeksler1[col].loc["2024-11-30"]/endeksler1[col].loc["2024-10-31"])-1)*100
            aylıık.loc["2024-12-31"]=((endeksler1[col].loc["2024-12-31"]/endeksler1[col].loc["2024-11-30"])-1)*100
            aylıık.iloc[-1]=np.round(((((hareketlimadde["Aylık Ortalama"].iloc[-1])/hareketlimadde["Aylık Ortalama"].loc[f"{oncekiyear}-{onceki}-24"]))-1)*100,2)
        ...
        for col in weighted_indices.columns:
            ...
            weighted_indices_aylık[col].iloc[-1]=np.round((((hareketli_aylik_ortalama(weighted_indices[col])["Aylık Ortalama"].iloc[-1])/(hareketli_aylik_ortalama(weighted_indices[col])["Aylık Ortalama"].loc[f"{oncekiyear}-{onceki}-24"]))-1)*100,2)



        weighted_indices_aylık=weighted_indices_aylık.loc["2024-11":]
        tarih=datetime.now(istanbul_tz).strftime("%Y-%m")
        oncekitarih=(datetime.now(istanbul_tz)-timedelta(days=30)).strftime("%Y-%m")
        ...
        for col in endekslerr.index:
            ...
            ay=datetime.now(istanbul_tz).month
            yıl=datetime.now(istanbul_tz).year

            ay=f"0{ay}"
            önceki_ay=(datetime.now(istanbul_tz)-timedelta(days=30)).month
            önceki_yıl=(datetime.now(istanbul_tz)-timedelta(days=30)).year
...
if page=="Harcama Grupları":
    ...
    degisim24harcama.iloc[-1]=np.round(((((hareketlimaharcama["Aylık Ortalama"].iloc[-1])/hareketlimaharcama["Aylık Ortalama"].loc[f"{oncekiyear}-{onceki}-24"]))-1)*100,2)
...
if page=="Özel Kapsamlı Endeksler":
    ...
    for col in tüik.columns:
        ...
        göstergeaylık[col].iloc[-1]=(((hareketli_aylik_ortalama(özelgöstergeler[col])["Aylık Ortalama"].fillna(method="ffill").iloc[-1]/hareketli_aylik_ortalama(özelgöstergeler[col])["Aylık Ortalama"].loc[f"{oncekiyear}-{onceki}-24"]))-1)*100
